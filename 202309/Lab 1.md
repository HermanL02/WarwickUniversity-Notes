...
The reason why deprecated key derivation used is a warning is because no salting is used in the encrypting algorithm, which makes the encryption vulnerable to brute force attacks. 
## 1.3 -pbkdf2
...
## 2.2 Diagrams of AES-128-CBC and ECB
...
The image of ECB has a clear shape of the original image. On the contrary, the image of CBC has no shape clearly shown. 

## Difference 
When encrypting the same colour (the same data in a more general case), the ECB mode generates the same encryption code, which means it cannot encrypt the patterns of data. It is not safe, because some features can still be seen even after the encryption. 
When encrypting the same data with CBC mode instead, it connects each part with the previous block, which prevents the attacker to see the pattern. 
