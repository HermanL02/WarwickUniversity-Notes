...
# Question 4
By using the diagram, we may observe the attacker sniffs the traffic and calculate the correct sequence number, then sends a data packet to the server, which has a sequence x and payload 8, which is within the predicted range, the server would think this packet is legit. 
