...
During this time, the user's session is still waiting to confirm the current sequence. It will cause the users TCP congestion control enabled and assumes the current packet loss, which prevents the user sending new data packets to the server. The users session may try to restore the session through keep sending the retransmission packets and waiting the correct ACK, and exhausted the resources and causing a freeze. 