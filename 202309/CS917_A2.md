...
## Part 2 
First, I create a dictionary that associate the morse code without the first character to the chars. Ex. ".-"->"A" to "-" -> "A". It costs O(1) time because the time to run it does not related to the input. 
...
Third, we use `itertools`, which is a default package imported by the professor. 
