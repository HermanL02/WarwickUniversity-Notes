...
## Part 2 
First, I create a dictionary that associate the morse code without the first character to the all possible letters. Ex. `".-"->"A", "--" ->"M"` TO `"-" -> ["A","M"]`. It costs O(1) time because the time to run it does not related to the input. 
Second, we read the dictionary file to list, and we only keep the words that have the same length as the input list. This step also cost O(1). 
Third, we translate the morse code to all possible chars and store it in `result`. Then we use `itertools`, which is a default package imported by the professor. 

> [The time complexity of generating combinations using `itertools. product()` is O(N^M). ](https://www.tutorialspoint.com/all-position-character-combination-using-python#:~:text=The%20time%20complexity%20of%20generating,M%20is%20the%20combination%20length.)
