...
To convert each morse code to a letter, we need to associate each to each. Therefore, a dictionary would be a ideal choice. It has more efficiency than a list. 
During the process, to increase the efficiency, I believe a divide and conquer approach is suitable to use. In a normal approach, if we decrypt the code through normal iteration, it would cost a O(n) time complexity. However, if we apply the divide and conquer, we can divide the list by half until there is only one letter left, after we turn this to letter, then we combine them to the new string. It would finally cost $O(nlogn)$ time complexity 
