...
Supposing one operation takes 1 microseconds, if one function costs $n^2$ time to solve n size problem, it uses $n^2$ microseconds. To determine by what size it can be done within 1 second, we can convert the time to microseconds, then create an equation to solve n. 
...
3. Since c = 3, $log_b(a) = log_{3/5}(4) = -2.71$. In this case, b is less than 1. By the definition of master theorem, Master Theorem applies when a>=1, b>1 and f(n) is asymptoticly positive
...
5. Since only Case 3 is used to compare $n^{log_b(a)}$ against f(n) where $f(n) = \Omega(n^c)$ Since c = 1, $log_b(a) = log_{4}(8) = 3/2$, $c<log_a(b)$.
   Therefore, no master theorem can be applied. 
# Question 4
