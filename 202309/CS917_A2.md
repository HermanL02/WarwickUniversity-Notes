...
We assign the start vertex as A as the question required. Therefore, we can see the distances from the starting point to B and C are 17 and 47 separately, and other distances here would be considered as infinity. The lowest value would be current distance add the distance A-B. 
Then, we assign B as the current vertex. Then we can calculate the 
## Result
