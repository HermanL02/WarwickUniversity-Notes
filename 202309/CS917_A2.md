...
### Explanation
Kruskal's algorithm creates two lists at the beginning. One contains all the edges. The other contains each node as a single node forest.
Our target is to combine all the single node forest as one large forest containing all the nodes. 
We keep finding the minimal edge. We observe if it connects the two nodes in the same forest. If it does not connect two nodes in the same forest, then we can combine the two node in their original forest to a new forest. If the minimal edge already exists in the same tree. (e.g. Tree: A-B-C, minimal edge connects AC.) Then we discard the edge and continue. 
Therefore, after the size of the forests list be 1. The forest is connected. 
# Question 8
```
