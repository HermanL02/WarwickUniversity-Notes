...
We assign the start vertex as A as the question required. Therefore, we can see the distances from the starting point to B and C are 17 and 47 separately, and other distances here would be considered as infinity. The lowest distance go to the next unvisited point would be current distance add the distance from A to B, which is 17.  
Then, we assign B as the current vertex, and we know the current distance is Then we can calculate the 
## Result
