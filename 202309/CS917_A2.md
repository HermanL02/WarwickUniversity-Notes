...
2. Since c = 2, $log_b(a) = log_{4}(16) =  2$, $c=log_a(b)$
   Therefore, we can apply Case 2. The complexity is $\Theta(n^2)$ by definition. 
3. Since c = 3, $log_b(a) = log_{3/5}(4) = -2.71$, $c>log_a(b)$
   Therefore, we can apply Case 3. The complexity is $\Theta(n^3)$ by definition. 
4. Since c < 2, $log_b(a) = log_{4}(2) =  1/2 $, we cannot determine the relationship between $c$ and $log_a(b)$. Therefore, none of them can be applied. 
5. Since c > 1, $log_b(a) = log_{4}(8) =  1.5$, $c>log_a(b)$
