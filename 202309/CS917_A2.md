...
Similar to the previous question, we assign the start vertex as A. Therefore, we can see the distances from the starting point to A's neighbours, B and C are 17 and 47 separately, and other distances here would be considered as infinity. The lowest distance go to the next unvisited point would be current distance add the distance from A to B, which is 17.  
## Result
