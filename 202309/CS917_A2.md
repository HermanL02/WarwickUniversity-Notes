...
During the process, to increase the efficiency, I believe a divide and conquer approach is suitable to use. In a normal approach, if we decrypt the code through normal iteration, it would cost a O(n) time complexity. However, if we apply the divide and conquer, we can divide the list by half until there is only one letter left, after we turn this to letter, then we combine them to the new string. It may take some time to combine the strings together, but I still think in this question a divide and conquer should 
