...
Third, we need to minimize the latency by simplifying code. The message delivery system, post-interception, encryption and decryption, operates without long latency. 
...
We need to perform continuous security assessments. We plan to use both automated methods, while we also plan to apply manual inspection techniques, to identify and check if there are any security issues. 
...
The first phase is the initial setup. I will spend two weeks to set up project goals, which also includes thinking about customers' requirements, and define the scope for both the Windows and Android platforms. During this time, we will set up the project environment in Jira, creating epics and stories for initial tasks. 
