...
On the Android platform, we decided to enhance an open source keyboard application by adding a new plugin that uses Kotlin. Our strategy takes advantage of the [fcitx5 - android] (https://github.com/fcitx5-android/fcitx5-android) application, it has a wide range of community support. Although there is still little amount of people can access GitHub, it is comparely still more accessable than other apps. This input method relies on local dictionaries and frequent updates, ensuring that user input is not shared with third-party entities. This framework provides a solid foundation for integrating E2EE(end-to-end encryption) capabilities.
