...
There are a few components. The first part is the context. It can create a context to provide the message status and management functions. The second part is the provider, which could have useState or useReducer to manage the message status. When the hook receives a new WeChat message, the provider will refresh the 
## Android
