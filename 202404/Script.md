...
### Using VPN
When I was in mainland, I was trying to use VPN to chat with my friends. I was concerned about lots of things. First, the connection is not stable, and people cannot always keep the VPN on, because some apps does not support foreign IP. (companies search app/some bank loan app/video app/ driver license management app, and so on)  I believe not only I, but also the other users are concerning about the quality of the connection. It prevents us from using it as the instant chat. After that, if I decide to talk about something legal but sensitive, I have to use WeChat to tell them lets open our VPN and move to another foreign app to chat. 
### Any products before? 
